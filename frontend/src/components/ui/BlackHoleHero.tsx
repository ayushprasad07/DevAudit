...
interface BlackHoleConfig {
  ...
  maxDpr: number;
  focus: [number, number];
  scrim: "left" | "right" | "top" | "bottom" | "none";
...
interface BlackHoleProps {
  ...
  className?: string;
  /** Inline style applied to the outer wrapper (use this to control size). */
  style?: React.CSSProperties;
  /** Base config values, merged under the built-in defaults. */
  base?: Partial<BlackHoleConfig>;
  /** Overrides applied when the viewport is >= 768px wide. */
  wide?: Partial<BlackHoleConfig>;
  /** Overrides applied when the viewport is < 768px wide. */
  narrow?: Partial<BlackHoleConfig>;
  /** Content layered into the same black-hole scene. */
  children?: React.ReactNode;
}

type ProgramInfo = { program: WebGLProgram; u: Record<string, WebGLUniformLocation | null> } | null;
type RenderTarget = { fb: WebGLFramebuffer; tex: WebGLTexture; w: number; h: number } | null;
...
const DEFAULT_BASE: BlackHoleConfig = {
  distance: 24,
  elevation: -5.5,
  azimuth: 0,
  orbitSpeed: 0,
...
const DEFAULT_WIDE: Partial<BlackHoleConfig> = {
  focus: [0.72, 0.46],
  scrim: "left",
  elevation: -5.5,
  fov: 42,
...
const DEFAULT_NARROW: Partial<BlackHoleConfig> = {
  focus: [0.5, 0.76],
  scrim: "top",
  elevation: -7,
  fov: 58,
...
/* ------------------------------------------------------------------------ */
/*  WebGL driver — mounts on a host element + canvas, returns a cleanup fn   */
/* ------------------------------------------------------------------------ */

function initBlackHole(
  host: HTMLDivElement,
  canvas: HTMLCanvasElement,
  baseProps: { base: BlackHoleConfig; wide: Partial<BlackHoleConfig>; narrow: Partial<BlackHoleConfig> }
): () => void {
  const reduced =
  ...
  function link(fragSrc: string): ProgramInfo {
  ...
  let sceneProg: ProgramInfo = null;
  let blendProg: ProgramInfo = null;
  let brightProg: ProgramInfo = null;
  let blurProg: ProgramInfo = null;
  let compProg: ProgramInfo = null;
  let vbo: WebGLBuffer | null = null;
  let scene: RenderTarget = null;
  let histA: RenderTarget = null;
  let histB: RenderTarget = null;
  let bloomA: RenderTarget = null;
  let bloomB: RenderTarget = null;
  let settled = 0;
  let width = 0;
  let height = 0;
  let sceneW = 0;
  ...
  function dropTargets() {
    [scene, histA, histB, bloomA, bloomB].forEach((t) => {
      if (!t) return;
  ...
  function resize() {
    ...
    const dpr = software ? 1 : Math.min(window.devicePixelRatio || 1, Math.max(1, current.maxDpr));
    const cssW = Math.max(1, Math.round(rect.width));
    const cssH = Math.max(1, Math.round(rect.height));
    const scale = software ? 0.34 : Math.min(1, Math.max(0.4, current.resolution));
  ...
  function pass(prog: ProgramInfo, target: RenderTarget) {
  ...
  function render(t: number) {
    ...
    const hlt = HALTON[settled % HALTON.length];
    gl!.uniform2f(u.uJitter, hlt[0] - 0.5, hlt[1] - 0.5);
    ...
    function blurStep(src: NonNullable<RenderTarget>, dst: NonNullable<RenderTarget>, dx: number, dy: number) {
      pass(blurProg, dst);
      bind(src.tex, 0);
  ...
  function tick(now: number) {
    ...
    lastFrame = now;
    if (!current.paused && !reduced) clock += dt;
    render(clock);
  }

  // Responsive config — mirrors the narrow/wide overrides.
  const narrowMQ = window.matchMedia("(max-width: 767px)");
  const current: BlackHoleConfig = { ...baseProps.base };

  function applyLayout(narrow: boolean) {
    Object.assign(current, narrow ? baseProps.narrow : baseProps.wide);
  }
  applyLayout(narrowMQ.matches);
  ...
  const ro = new ResizeObserver(() => {
    resize();
    if (reduced || current.paused) settle(16);
  });
  ro.observe(host);
  ...
  else (narrowMQ as any).addListener(onMQ);
  ...
  return function cleanup() {
    ...
    if (narrowMQ.removeEventListener) narrowMQ.removeEventListener("change", onMQ);
    else (narrowMQ as any).removeListener(onMQ);
    dropTargets();
    if (vbo) gl!.deleteBuffer(vbo);
    [sceneProg, blendProg, brightProg, blurProg, compProg].forEach((p) => {
...
export default function BlackHole({ className, style, base, wide, narrow, children }: BlackHoleProps) {
  ...
  useEffect(() => {
    ...
    const mergedBase: BlackHoleConfig = { ...DEFAULT_BASE, ...base };
    const mergedWide: Partial<BlackHoleConfig> = { ...DEFAULT_WIDE, ...wide };
    const mergedNarrow: Partial<BlackHoleConfig> = { ...DEFAULT_NARROW, ...narrow };

    const cleanup = initBlackHole(host, canvas, {
      base: mergedBase,
      wide: mergedWide,
      narrow: mergedNarrow,
    });
  ...
  return (
    ...
    >
      ...
      />
      {children ? <div className="black-hole-content">{children}</div> : null}
    </div>
  );
}
